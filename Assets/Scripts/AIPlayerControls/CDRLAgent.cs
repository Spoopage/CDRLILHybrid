...
public class CDRLAgent : Agent
{
    ...
    // Memecah hashset untuk kebutuhan komparasi grafik di TensorBoard
    ...
    public string coverageFileName = "CoverageData_CDRL.csv";
    ...
    public string positionFileName = "PositionLog_CDRL_MapB.csv";
    ...
    public override void OnEpisodeBegin()
    {
        ...
        // Simpan koordinat ke CSV
        SaveCoverageToFile();

        // Clear memory untuk episode berikutnya
    ...
    public override void OnActionReceived(ActionBuffers actions)
    {
        ...
        totalActions++;
        //Debug.Log($"Actions: moveZ={actions.DiscreteActions[0]}, rotateY={actions.DiscreteActions[1]}, sprint={actions.DiscreteActions[2]}, jump={actions.DiscreteActions[3]}");
    ...
    private void ProcessMovement(ActionBuffers actions)
    {
        ...
        //Debug.Log($"[MOVE] grounded={grounded}, moveInput={moveInput}, speed={speed}, moveDir={moveDir}, rb.velocity={rb.linearVelocity}");

        float forceMult = grounded ? 10f : 10f * airMultiplier;
        rb.AddForce(moveDir.normalized * speed * forceMult, ForceMode.Force);
        //Debug.Log($"[AFTER_FORCE] velocity={rb.linearVelocity}, force={moveDir.normalized * speed * forceMult}");
        ...
        //Debug.Log($"[JUMP_CHECK] isJumping={isJumping}, readyToJump={readyToJump}, grounded={grounded}");
        // Lompat
    ...
    private void TrackGridCoverage()
    {
        ...
        {
            // Cek jenis area untuk filter data grafik TensorBoard
            if (mapOrganizer.IsGoldenPathArea(currentGrid))
            {
                if (goldenPathGrids.Add(currentGrid))
                {
                    // Imbalan murni CDRL (statis 0.1f)
                    AddReward(0.1f);
                }
            }
            else
    ...
    private void DetectAnomalies()
    {
        ...
        if (transform.position.y < -10f)
        {
            ...
            // PENALTI: Mencegah agen sengaja bunuh diri berulang kali
            SetReward(-1f);
            EndEpisode();
        }
